...
from infrastructure.database.weaviate import WeaviateVectorService, WeaviateConfig
...
def make_service_with_in_memory_client(embed_fn=None):
    cfg = WeaviateConfig(url="http://fake", api_key=None, timeout=5)
    svc = WeaviateVectorService(config=cfg, embed_fn=embed_fn or (lambda text: [0.3, 0.4, 0.5]))
...
def test_delete_removes_item_from_storage():
    ...
    deleted_count = svc.delete_knowledge_items_by_sqlite_id(sqlite_id=88)
    assert deleted_count == 1
    assert len(client.storage) == 0

    items = svc.get_knowledge_items(sqlite_id=88, limit=5)
    assert items == []
