...
{{
  ...
  "explanation": "对任务规划的说明"
}}

## 处理规则
- **明确意图**：如果用户明确请求代码分析或数据库操作，生成对应的任务规划
- **信息不足**：如果用户意图明确但信息不足，在explanation中说明需要补充的信息
- **无法识别**：如果完全无法识别意图，设置空数组并在explanation中说明为什么无法从本次输入中解析出代码评审或数据库任务,指导用户补充信息

## 输出要求
- 必须输出合法的JSON格式
- 如果无相关任务，必须设置空数组，格式如下
  {{
    "code_analysis_tasks": []
    "db_tasks": []
    "explanation": "意图识别失败，需要与用户进行需求澄清"
  }}
- explanation必须清晰说明识别结果和下一步建议
